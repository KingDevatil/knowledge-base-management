...
图谱功能此前只负责生成 HTML/JSON 可视化，不参与搜索，所以对召回没有实质提升。现在构图会建立文档与核心实体的声明边、共享核心实体边、共享标签、同目录和可选语义相似度边，并生成 `kbdata/graph/retrieval_index.json`；检索链路按种子数、最大跳数、边权阈值和总权重受控扩展。图谱不存在、过期或超时会自动降级，不影响三条基础通道。文档批量变化后应在后台或通过 `build_knowledge_graph` 重新构建；建议启用语义关系时使用 `semantic_threshold=0.72` 起步，再按误召回情况调高。
...
| `tags` / `entities` / `updated_at` | 文档标签、头部声明的核心实体和更新时间 |
