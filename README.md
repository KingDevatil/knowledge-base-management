...
| **用户角色** | 四级权限体系：超级管理员、管理员、编辑者、只读用户，支持路径级访问控制 |
| **知识图谱** | 自动构建文档关系图谱（标签共享、目录结构、语义相似），交互式可视化 |
| **文档分享** | 基于 Token 的文档分享链接，支持设定过期时间，可随时吊销 |
| **双层认证** | API Key（Agent 调用）+ Session（Web 后台） |
...
> - 在启动器中勾选"使用 .env 文件配置"可将 `.env` 中的变量覆盖启动器默认值
>
> #### 启动器界面说明
>
> `kb_launcher.pyw` 提供 3 个标签页：
>
> **① 服务状态** — 显示 5 个服务（Ollama / Redis / Chroma / MinIO / MCP Gateway）的运行状态、端口、操作按钮，每 3 秒自动轮询健康检查。支持一键"启动全部"和"停止全部"，也可单独启停某个服务。底部有"使用 .env 文件配置"复选框和 Gateway 专用的"清除缓存并重启"按钮。
>
> **② 接入地址** — 展示管理后台各页面链接、MCP 端点地址、Agent 配置代码块（带复制按钮：Cursor/Windsurf 配置、Claude Desktop/Kimi Code mcp-proxy 中转配置）、REST API 端点、MinIO 控制台地址。
>
> **③ 运行日志** — 实时滚动显示 Gateway 日志，后台线程异步读取 `gateway.log`，支持一键清空。
>
> 关闭窗口最小化到系统托盘（需 `pystray` + `Pillow`），服务继续在后台运行。托盘右键菜单可"显示窗口"/"启动全部"/"停止全部"/"退出"。
>
> **常见问题**：
> - Memurai 安装后需重启终端（或刷新环境变量）才能被 `redis-cli` 识别
> - 如果 Ollama 启动后端口冲突，检查是否已有 Ollama 托盘图标在运行
> - 所有服务日志写入 `kbdata/logs/`，启动器界面同步显示
...
系统内置 4 个用户角色，从管理员后台 `/admin/users`（需管理员权限）管理：

| 角色 | 仪表盘 | 文档查看 | 文档编辑 | 用户管理 | API Key 管理 | 系统设置 |
|------|--------|---------|---------|---------|------------|---------|
| `super_admin` | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ |
| `admin` | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ |
| `user` | ✗ | ✓¹ | ✓¹ | ✗ | ✗ | ✗ |
...
¹ 受 `authorized_paths` 路径白名单限制，仅可访问被授权的目录

### 路径级权限

`user` 和 `viewer` 角色可通过 `authorized_paths` 字段限定可访问的目录范围。留空则无权访问任何文档，指定后仅可访问匹配路径及其子路径下的文档。

### 用户管理操作

| 操作 | 路径 | 说明 |
|------|------|------|
| 用户列表 | `GET /admin/users` | 查看所有用户（不含密码哈希） |
| 创建用户 | `POST /admin/api/users/create` | 指定用户名、密码、角色、授权路径 |
| 更新用户 | `POST /admin/api/users/{username}/update` | 修改角色、授权路径或密码 |
| 删除用户 | `POST /admin/api/users/{username}/delete` | 最后一个 `super_admin` 不可删除 |

### 安全保护

- 至少保留一个 `super_admin`，最后一个超级管理员不可降级或删除
- 普通用户无法将自己升级为管理员（需 admin 操作）
- 密码通过 bcrypt 哈希存储
...
知识图谱将知识库中零散的文档自动组织成关系网络，帮助发现文档间的隐性关联。

### 关系类型

| 关系 | 来源 | 说明 |
|------|------|------|
| `co_tag` | 元数据 | 共享至少一个标签的两篇文档 |
| `same_directory` | 目录结构 | 在同一目录下的文档 |
| `semantically_similar` | 向量相似度 | 语义余弦相似度超过阈值（可选，默认关闭） |

### 构建流程

1. 从知识库获取所有文档元数据
2. 建立节点（每文档一个）+ 边（标签共享 + 可选语义相似）
3. Louvain 社区聚类算法自动分组
4. 为每个社区生成可读标签
5. 导出交互式 HTML 图和 JSON 数据

### MCP 工具

| 工具名 | 描述 | 参数 |
|--------|------|------|
| `build_knowledge_graph` | 构建知识图谱，分析文档关系，生成交互式可视化 | `semantic_threshold`（0.0=关闭，0.7=推荐） |

### Web 管理

- **查看图谱**：`GET /admin/graph` — 交互式可视化页面（所有登录用户可访问）
- **重建图谱**：`POST /admin/api/rebuild-graph` — 手动触发重新构建（需编辑权限）
- **语义阈值**：在系统设置页面 `/admin/settings` 中配置

### 输出文件

图谱数据保存在 `kbdata/graph/` 目录下：
- `graph.html` — 交互式可视化（可直接用浏览器打开）
- `graph.json` — 原始图谱数据（可被外部程序使用）
- `.graphify_labels.json` — 社区标签

---

## 文档分享

每篇文档可生成独立的分享链接，通过 Token 鉴权，无需登录即可查看。

### 使用方式

1. 打开一篇文档，点击"分享"按钮
2. 系统生成唯一的 48 位十六进制 Token
3. 复制分享链接发给他人
4. 可在文档详情页查看已创建的分享链接列表
5. 随时吊销（Token 立即失效）
...
| 操作 | 路径 | 说明 |
|------|------|------|
| 创建分享 | `POST /api/documents/{doc_id}/share/create` | 生成分享 Token |
| 吊销分享 | `POST /api/documents/{doc_id}/share/revoke` | 使 Token 立即失效 |
| 分享列表 | `GET /api/documents/{doc_id}/shares` | 查看文档的所有分享链接 |

公开查看路由（无需认证）：`GET /share/{token}` — 通过 Token 访问分享的文档内容。
...
│       ├── server.py               # MCP 服务器（工具注册 + 请求分发）
│       ├── tools.py                # MCP 工具实现
│       ├── tools_reader.py         # 只读 MCP 工具（read 权限分离）
│       ├── knowledge_base.py       # Chroma 封装
│       ├── kb_graph.py             # 知识图谱构建器
│       ├── source_store.py         # MinIO 封装
│       ├── local_store.py          # 本地文件系统降级存储
│       ├── embedding.py            # Ollama Embedding
│       ├── chunker.py              # Markdown 切片
│       ├── auth.py                 # API Key 认证
│       ├── admin_auth.py           # 管理员认证 + 账户 CRUD
│       ├── lock.py                 # Redis 分布式锁
│       ├── directory_tree.py       # 目录树工具
│       ├── directory_store.py      # 目录持久化存储
│       ├── config.py               # 配置管理（BaseSettings）
│       ├── backup_sources.py       # 源文件备份工具
│       ├── logger.py               # 日志配置
│       ├── middleware.py           # 请求日志 / CSRF 中间件
│       ├── reset_admin_password.py # 管理员密码重置脚本
│       └── admin/                  # 后台管理（模块化路由）
│           ├── __init__.py
│           ├── routes.py           # 聚合路由（统一导出）
│           ├── routes_pages.py     # HTML 页面路由
│           ├── routes_api.py       # 登录/账户 API
│           ├── routes_documents_api.py  # 文档 CRUD API
│           ├── routes_admin_misc.py     # 系统设置/备份/图谱 API
│           ├── routes_shares_api.py     # 分享 API
│           ├── user_routes.py           # 用户管理路由
│           ├── share_routes.py          # 分享公开查看路由
│           └── helpers.py               # 模板/依赖注入/权限辅助
│           └── templates/
│               ├── base.html           # 全局布局（深色主题）
│               ├── dashboard.html      # 仪表盘
│               ├── login.html          # 登录页
│               ├── documents.html      # 文档列表
│               ├── document_view.html  # 文档阅读（含 TOC 侧边栏）
│               ├── document_edit.html  # 文档编辑
│               ├── upload.html         # 批量上传
│               ├── directories.html    # 目录管理
│               ├── graph.html          # 知识图谱可视化
│               ├── api_keys.html       # API Key 列表
│               ├── api_key_create.html # 新建 API Key
│               ├── users.html          # 用户管理
│               ├── share_view.html     # 分享页面（公开，无需登录）
│               ├── settings.html       # 系统设置
│               └── account.html        # 个人账户
├── kb_launcher.pyw                # 图形启动器（tkinter 界面）
├── plan.md / overview.md           # 设计文档
├── start-dev.bat                    # Windows 快捷启动
├── stop-dev.bat                     # Windows 快捷停止
├── start-gui.bat                    # 图形启动器快捷入口
├── init-config.bat                  # 初始化配置（生成 SESSION_SECRET）
...
从 MinIO 将原始 Markdown 文件按目录结构拉到本地，不含向量数据，适合增量备份和迁移：
...
# 或本地开发环境
cd mcp-gateway
python src/backup_sources.py
python src/backup_sources.py --output D:/backups/kb-20260522
```

备份脚本自动创建以时间戳命名的目录（如 `kb-sources-20260522_143000`），按 `{目录}/{文档ID}.md` 结构保存。

#### ② Web 后台导出（ZIP 包）

管理员登录后台，访问 `GET /admin/api/backup/export`，一键下载知识库所有文档为 `.md` ZIP 压缩包，适合快速迁移到其他系统。
...
### Q: Windows 启动时 ChromaDB 连接失败？

A: 通常是系统代理环境导致 Python httpx 走代理连接 localhost ChromaDB 被拒。排查步骤：

1. 检查是否开启了代理软件（VPN / Clash / V2Ray 等），尝试关闭
2. 检查环境变量 `NO_PROXY` 是否包含 `localhost` 和 `127.0.0.1`
3. 检查 Git 全局代理 `git config --global --get http.proxy`，如存在且代理服务未运行可移除：`git config --global --unset http.proxy`

### Q: Windows 启动器可以同时运行多个实例吗？

A: 启动器会自动清理残留的 MCP Gateway 进程后启动新实例。但多个实例同时尝试管理同一服务可能导致端口冲突，建议先"停止全部"再启动新实例。
