...
- 使用向量、关键词、标题/路径结构三路召回，并可用加权知识图谱扩展关联文档。
...
图谱功能此前只负责生成 HTML/JSON 可视化，不参与搜索，所以对召回没有实质提升。现在构图会保留共享标签数、同目录关系和语义相似度权重，并生成 `kbdata/graph/retrieval_index.json`；检索链路按种子数、最大跳数、边权阈值和总权重受控扩展。图谱不存在、过期或超时会自动降级，不影响三条基础通道。文档批量变化后应在后台或通过 `build_knowledge_graph` 重新构建；建议启用语义关系时使用 `semantic_threshold=0.72` 起步，再按误召回情况调高。
